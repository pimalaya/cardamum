...
The shared surface on Fastmail CardDAV passes end to end: `addressbook` (list / create / update / delete, all flags including clearing, `--json`, aliases, backend selection) and `card` (list / create / read / update / delete, paging, `--json`, `--if-match`), with correct exit codes and a clean read-back after every mutation. The two defects this run found, **F5** (`card update` on an absent id fabricating a card) and **F4** (metadata clearing being a no-op), were both fixed the same day and re-verified live, along with a regression sweep over the surrounding commands; see the [carddav-write-guards](../../log/2026-08-09-carddav-write-guards.md) log entry. Everything the 2026-07-17 run fixed is still fixed. **Release-ready for the CardDAV surface**, with the io-webdav removal work now landed upstream. Only cosmetics remain open: the raw error bodies (O1) and the no-op-reports-success case (O2).
