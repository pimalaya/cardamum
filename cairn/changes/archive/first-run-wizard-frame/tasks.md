...
- [x] Add `offer_configuration`, raised by a bare `cardamum` and by account resolution, returning whether the wizard ran.
- [x] Make a bare `cardamum` show the help when a configuration exists, or when `--account` names one, instead of always running the wizard.
- [x] Make account resolution a hook: drop the `exit(0)`, re-read the configuration, then fail the ordinary way when nothing landed.
...
- [x] Give `AccountConfig` the ordering-aware `render`, and drop the `GeneratedConfig` wrapper.
...
- [x] Fold into cairn/spec/wizard.md, superseding "The generated account is never the default"; log; land.
