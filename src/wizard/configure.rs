...
pub fn print_welcome(path: &Path) {
    ...
    eprintln!();
    eprintln!("Cardamum talks to your existing address books over CardDAV, JMAP, Google");
    eprintln!("People, Microsoft Graph or a local folder. It needs one account to know");
    eprintln!("which address books to read, and no configuration file was found at:");
    eprintln!();
...
mod tests {
    ...
    fn config_path() -> PathBuf {
        ...
        env::temp_dir().join(format!("cardamum-configure-{id}.toml"))
    }
    ...
    fn account(default: bool) -> AccountConfig {
        AccountConfig {
            default,
            vdir: Some(crate::config::VdirConfig {
                home_dir: String::from("/tmp/contacts"),
            }),
            ..Default::default()
        }
    }
    ...
    fn a_generated_account_parses_back() {
        ...
        assert_eq!(
            account.vdir.as_ref().map(|config| config.home_dir.as_str()),
            Some("/tmp/contacts")
        );
    ...
    fn the_endpoint_reads_before_its_credentials() {
        ...
        let carddav: Vec<&str> = document
            .lines()
            .filter(|line| line.starts_with("carddav."))
            .collect();

        assert_eq!(carddav[0], "carddav.server = \"https://dav.example.org\"");
    }
    ...
    fn a_taken_name_gets_a_suffix() {
        let existing = ExistingConfig {
            names: vec![String::from("perso"), String::from("perso-2")],
            has_default: true,
        };
