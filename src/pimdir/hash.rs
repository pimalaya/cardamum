...
//! content-addressed so a card present in several places is stored once.
//!
//! This digest MUST match Neverest's (neverest/src/offline/hash.rs) and
//! Himalaya's (himalaya/src/pimdir/hash.rs), a 128-bit FNV-1a variant rendered
//! as 32 hex chars, so every consumer agrees on object identity and a card
//! Cardamum adds deduplicates against the same card a sync stored.
...
mod tests {
    ...
    fn matches_the_shared_digest_shape() {
        ...
        let hash = content_hash(b"BEGIN:VCARD\r\nUID:x\r\nEND:VCARD\r\n");
        assert_eq!(hash.0.len(), 32);
        assert!(hash.0.chars().all(|char| char.is_ascii_hexdigit()));
