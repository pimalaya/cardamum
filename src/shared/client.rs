...
impl AddressbookClient {
    ...
    pub fn new(
        ...
        #[cfg(feature = "jmap")]
        if inner.is_none() && backend.allows_jmap() {
        ...
        #[cfg(feature = "msgraph")]
        if inner.is_none() && backend.allows_msgraph() {
        ...
        #[cfg(feature = "google")]
        if inner.is_none() && backend.allows_google() {
            if let Some(google_config) = account_config.google.take() {
