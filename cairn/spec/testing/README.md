...
The 2026-08-09 vdir re-run, in a throwaway local instance, found the specific surface clean and **six defects on the shared arm**, four of them new: `card update` on an absent id created the file, `card create -k <unknown>` created the collection, `card list -k <unknown>` listed as empty with exit 0, and `addressbook delete -k <unknown>` leaked a raw OS error, plus the two carried over from July (clearing metadata was a no-op, `--if-match` was silently ignored). All were the local twins of what the network backends were fixed for today: the shared vdir commands wrote through to the filesystem without first checking that what they addressed exists. All six are fixed and re-verified, the clearing one through io-vdir, whose collection update now removes the metadata file of a value it is not given; see the [vdir-write-guards](../../log/2026-08-09-vdir-write-guards.md) log entry.

**Open, and not backend-specific:** `account check` exits 0 even when a backend reports FAIL, so a script cannot tell a healthy account from a broken one without parsing the output (V9 in [vdir-local.md](vdir-local.md)).
