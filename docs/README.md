...
- [testing/carddav-specific.md](testing/carddav-specific.md): CardDAV specific API (flat WebDAV methods) — full surface validated on Fastmail incl. incremental `report sync`, read + card round-trip on iCloud; `discover` on home-configured accounts (C1) and the raw-XML escape hatch deferred.
- [testing/msgraph-specific.md](testing/msgraph-specific.md): Microsoft Graph specific API (nested by resource) — full contact-folder/contact/profile surface validated live incl. raw Graph JSON in/out and the `contact delta` incremental sync; no provider bugs.
