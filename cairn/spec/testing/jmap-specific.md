...
| `address-book create` / `update` / `destroy` | n/a | ⛔ Fastmail forbids AddressBook mutations (J1); surfaced cleanly. Commands are correct (work on a server that allows it). |
...
| `contact-card copy` | n/a | implemented, **not live-tested** (needs a second JMAP account) |
